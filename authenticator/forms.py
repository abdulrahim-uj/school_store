...
class UserRegistrationForm(UserCreationForm):
    ...
    def clean_username(self):
        ...
        existing = User.objects.filter(username__iexact=self.cleaned_data['username'].lower())
        if existing.exists():
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email__iexact=self.cleaned_data['email'], is_active=True):
            ...
            raise forms.ValidationError(_("Registration using %s email addresses is not allowed. Please supply a "
                                          "different email address." % email_domain))
